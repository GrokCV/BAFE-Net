...
def tpfp_noco(det_centroids, gt_noco_map, gt_bboxes, noco_thr):
    ...
    for i in sort_inds:
        ...
        if crow > img_hei or ccol > img_wid:
            # print(1)
            noco_val = 0
        else:
            if crow == img_hei:
...
def eval_mnocoap(
    det_centroids, gt_noco_maps, gt_bboxes, noco_thr=0.5, logger=None, nproc=4
):
    ...
    for det_centroid, gt_noco_map, gt_bbox, noco_thr in zip(
        det_centroids, gt_noco_maps, gt_bboxes, [noco_thr for _ in range(num_imgs)]
    ):
